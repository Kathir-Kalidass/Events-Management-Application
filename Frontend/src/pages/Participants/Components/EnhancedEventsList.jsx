...
const EnhancedEventsList = ({ onDataChange }) => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      // Fetch all events
      const eventsResponse = await fetch('http://localhost:5050/api/participant/events', {
        headers: {
      ...
      const registrationsResponse = await fetch(`http://localhost:5050/api/participant/my-events/${participantId}`, {
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json",
        },
  ...
  const handleRegister = async (eventId) => {
    try {
      ...
      const response = await fetch('http://localhost:5050/api/participant/register', {
        method: 'POST',
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
