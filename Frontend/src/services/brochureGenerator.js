...
export const generateEventBrochure = async (event) => {
  try {
    ...
    let currentY = margin;
    const lineHeight = 4.5; // Reduced for more conciseness
    const sectionSpacing = 6; // Reduced for more conciseness
  ...
  const addWrappedText = (text, x, y, maxWidth, fontSize = 11, style = 'normal') => {
    ...
    checkPageBreak(textHeight + 3); // Reduced from 5
  ...
  const addSectionHeader = (title) => {
    checkPageBreak(15); // Reduced from 20
    doc.setFillColor(240, 240, 240);
    doc.rect(margin, currentY - 2, contentWidth, 8, 'F'); // Reduced height from 10 to 8
    
    doc.setFontSize(11); // Reduced from 12
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(0, 0, 0);
    doc.text(title, margin + 3, currentY + 4); // Adjusted position
    currentY += 10; // Reduced from 12
  };

  // Two-column layout helper
  const addTwoColumnSection = (leftContent, rightContent) => {
    const colWidth = (contentWidth - 8) / 2; // Reduced gap from 10 to 8
    const leftX = margin;
    const rightX = margin + colWidth + 8; // Reduced gap from 10 to 8
    ...
    leftContent.forEach(item => {
        if (item.type === 'header') {
          ...
          addWrappedText(item.text, leftX, currentY, colWidth, 9); // Reduced from 10
        }
      });
    ...
    rightContent.forEach(item => {
        if (item.type === 'header') {
          ...
          addWrappedText(item.text, rightX, currentY, colWidth, 9); // Reduced from 10
        }
      });
    
    currentY = Math.max(leftEndY, currentY) + 4; // Reduced spacing from sectionSpacing
  };
...
This program is ideal for professionals looking to **enhance their career prospects**, students seeking to supplement their academic knowledge with practical skills, and organizations aiming to **upskill their workforce** in emerging technologies.`;
  ...
  // Parse and format the enhanced description with bold text
  const formatEnhancedText = (text) => {
    ...
    paragraphs.forEach(paragraph => {
      ...
      // Check if this is a header line (starts and ends with **)
      if (paragraph.startsWith('**') && paragraph.endsWith('**') && paragraph.indexOf('**', 2) === paragraph.length - 2) {
        const cleanHeader = paragraph.replace(/\*\*/g, '');
        addWrappedText(cleanHeader, margin, currentY, contentWidth, 10, 'bold');
        currentY += 2; // Small spacing after headers
      } else {
        // Handle paragraph with inline bold formatting
        ...
        segments.forEach((segment, index) => {
          if (index % 2 === 0) {
            ...
            // This would be bold text, but since jsPDF doesn't support inline formatting,
            // we'll convert it to uppercase for emphasis
            formattedText += segment.toUpperCase();
          }
        });
        
        addWrappedText(formattedText, margin, currentY, contentWidth, 9);
  ...
  // Course Syllabus/Topics Section (if available in event data)
  if (event.syllabus || event.topics || event.modules) {
    addSectionHeader('COURSE CURRICULUM');
    
    const syllabusContent = event.syllabus || event.topics || event.modules || 
      generateIntelligentSyllabus(event);
    
    addWrappedText(syllabusContent, margin, currentY, contentWidth, 9);
    currentY += 4; // Reduced spacing
  }

  // Intelligent syllabus generator function
  function generateIntelligentSyllabus(event) {
    const title = event.title || 'Course';
    const duration = event.duration || '3 days';
    const titleLower = title.toLowerCase();
    
    // Extract number of days for curriculum structure
    const durationMatch = duration.match(/(\d+)\s*(day|week|hour)/i);
    const numDays = durationMatch ? parseInt(durationMatch[1]) : 3;
    const unit = durationMatch ? durationMatch[2].toLowerCase() : 'day';
    ...
    if (titleLower.includes('ai') || titleLower.includes('artificial intelligence') || titleLower.includes('machine learning')) {
      syllabus = generateAISyllabus(numDays, unit);
    } else if (titleLower.includes('data') || titleLower.includes('analytics') || titleLower.includes('science')) {
      syllabus = generateDataScienceSyllabus(numDays, unit);
    } else if (titleLower.includes('web') || titleLower.includes('frontend') || titleLower.includes('backend') || titleLower.includes('full stack')) {
      syllabus = generateWebDevSyllabus(numDays, unit);
    } else if (titleLower.includes('cyber') || titleLower.includes('security') || titleLower.includes('ethical hacking')) {
      syllabus = generateCyberSecuritySyllabus(numDays, unit);
    } else if (titleLower.includes('cloud') || titleLower.includes('aws') || titleLower.includes('azure') || titleLower.includes('devops')) {
      syllabus = generateCloudDevOpsSyllabus(numDays, unit);
    } else if (titleLower.includes('mobile') || titleLower.includes('android') || titleLower.includes('ios') || titleLower.includes('app')) {
      syllabus = generateMobileSyllabus(numDays, unit);
    } else if (titleLower.includes('blockchain') || titleLower.includes('crypto')) {
      syllabus = generateBlockchainSyllabus(numDays, unit);
    } else if (titleLower.includes('iot') || titleLower.includes('internet of things') || titleLower.includes('embedded')) {
      syllabus = generateIoTSyllabus(numDays, unit);
    } else {
      syllabus = generateGenericSyllabus(numDays, unit, title);
    }
    
    return syllabus;
  }

  function generateAISyllabus(days, unit) {
    if (days <= 1) {
      return `**Day 1:** Introduction to AI/ML • Fundamentals of Machine Learning • Python for AI • Basic Neural Networks • Hands-on with Simple Models • Q&A Session`;
    } else if (days <= 3) {
      return `**Day 1:** Introduction to AI/ML • Machine Learning Fundamentals • Python Libraries (NumPy, Pandas) • Data Preprocessing

**Day 2:** Supervised Learning • Classification and Regression • Neural Networks Basics • Deep Learning Introduction

**Day 3:** Advanced Topics • Computer Vision/NLP Basics • Model Evaluation • Project Implementation • Future Trends`;
    } else {
      return `**Module 1:** AI/ML Foundations (${Math.ceil(days/4)} ${unit}s)
• Introduction to Artificial Intelligence and Machine Learning
• Mathematical foundations and statistics
• Python programming for AI/ML

**Module 2:** Core Machine Learning (${Math.ceil(days/4)} ${unit}s)
• Supervised and Unsupervised Learning
• Classification, Regression, and Clustering
• Model Selection and Evaluation

**Module 3:** Deep Learning (${Math.ceil(days/4)} ${unit}s)
• Neural Networks and Deep Learning
• Convolutional and Recurrent Neural Networks
• Popular Frameworks (TensorFlow, PyTorch)

**Module 4:** Advanced Applications (${Math.ceil(days/4)} ${unit}s)
...
• Industry Best Practices and Deployment`;
  ...
  function generateDataScienceSyllabus(days, unit) {
    if (days <= 1) {
      return `**Day 1:** Data Science Overview • Statistical Foundations • Python/R for Data Science • Data Visualization • Exploratory Data Analysis • Mini Project`;
    } else if (days <= 3) {
      return `**Day 1:** Data Science Fundamentals • Statistical Analysis • Python/R Programming • Data Collection and Cleaning

**Day 2:** Data Analysis and Visualization • Exploratory Data Analysis • Statistical Modeling • Data Mining Techniques

**Day 3:** Advanced Analytics • Predictive Modeling • Business Intelligence • Project Presentation • Career Guidance`;
    } else {
      return `**Module 1:** Data Science Foundations (${Math.ceil(days/4)} ${unit}s)
• Introduction to Data Science and Analytics
• Statistical methods and probability
• Programming languages (Python/R)

**Module 2:** Data Processing (${Math.ceil(days/4)} ${unit}s)
• Data Collection and Acquisition
• Data Cleaning and Preprocessing
• Database Management and SQL

**Module 3:** Analysis and Modeling (${Math.ceil(days/4)} ${unit}s)
...
• Capstone Project and Industry Case Studies`;
  ...
  function generateWebDevSyllabus(days, unit) {
    if (days <= 1) {
      return `**Day 1:** Web Development Basics • HTML5, CSS3, JavaScript • Responsive Design • Basic Project • Modern Development Tools`;
    } else if (days <= 3) {
      return `**Day 1:** Frontend Fundamentals • HTML5, CSS3, JavaScript • Responsive Web Design • Version Control (Git)

**Day 2:** Modern Frontend • JavaScript Frameworks (React/Vue/Angular) • API Integration • State Management

**Day 3:** Full-Stack Development • Backend Basics (Node.js/Python) • Database Integration • Deployment Strategies`;
    } else {
      return `**Module 1:** Frontend Development (${Math.ceil(days/3)} ${unit}s)
• HTML5, CSS3, and Modern JavaScript
• Responsive Design and CSS Frameworks
• Frontend Build Tools and Workflows

**Module 2:** Advanced Frontend (${Math.ceil(days/3)} ${unit}s)
• JavaScript Frameworks (React, Vue, or Angular)
• State Management and API Integration
• Testing and Performance Optimization

**Module 3:** Full-Stack Development (${Math.ceil(days/3)} ${unit}s)
...
• Authentication, Security, and Deployment`;
  ...
  function generateCyberSecuritySyllabus(days, unit) {
    return `**Module 1:** Security Fundamentals • Cybersecurity Overview • Threat Landscape • Risk Assessment • Security Policies

**Module 2:** Network Security • Network Protocols and Security • Firewalls and Intrusion Detection • Vulnerability Assessment

**Module 3:** Ethical Hacking • Penetration Testing Methodologies • Security Tools and Techniques • Incident Response

**Module 4:** Advanced Topics • Digital Forensics • Compliance and Governance • Emerging Security Threats`;
  }

  function generateCloudDevOpsSyllabus(days, unit) {
    return `**Module 1:** Cloud Fundamentals • Cloud Computing Concepts • Major Cloud Platforms (AWS/Azure/GCP) • Cloud Architecture

**Module 2:** DevOps Practices • CI/CD Pipelines • Infrastructure as Code • Containerization (Docker, Kubernetes)

**Module 3:** Cloud Services • Cloud Storage and Databases • Serverless Computing • Microservices Architecture

**Module 4:** Monitoring and Security • Cloud Monitoring and Logging • Security Best Practices • Cost Optimization`;
  }

  function generateMobileSyllabus(days, unit) {
    return `**Module 1:** Mobile Development Basics • Mobile Platforms Overview • Development Environment Setup • UI/UX Design Principles

**Module 2:** Native Development • iOS/Android Development • Platform-specific Features • App Store Guidelines

**Module 3:** Cross-Platform Development • React Native/Flutter • Hybrid App Development • API Integration

**Module 4:** Advanced Topics • Performance Optimization • App Security • Testing and Deployment Strategies`;
  }

  function generateBlockchainSyllabus(days, unit) {
    return `**Module 1:** Blockchain Fundamentals • Distributed Ledger Technology • Cryptography Basics • Consensus Mechanisms

**Module 2:** Smart Contracts • Ethereum Platform • Solidity Programming • DApp Development

**Module 3:** Advanced Applications • DeFi and NFTs • Enterprise Blockchain • Interoperability

**Module 4:** Implementation • Blockchain Development Tools • Testing and Deployment • Real-world Use Cases`;
  }

  function generateIoTSyllabus(days, unit) {
    return `**Module 1:** IoT Foundations • Internet of Things Overview • Sensors and Actuators • Communication Protocols

**Module 2:** Hardware Programming • Microcontrollers (Arduino/Raspberry Pi) • Embedded Programming • Circuit Design

**Module 3:** Connectivity and Data • Wireless Communication • Cloud Integration • Data Analytics

**Module 4:** Applications • Smart Home/City Projects • Industrial IoT • Security and Privacy Considerations`;
  }

  function generateGenericSyllabus(days, unit, title) {
    return `**Module 1:** Introduction and Fundamentals (${Math.ceil(days/4)} ${unit}s)
• Overview of ${title}
• Basic concepts and principles
• Industry applications and use cases

**Module 2:** Core Concepts (${Math.ceil(days/4)} ${unit}s)
• Detailed technical coverage
• Hands-on practical exercises
• Tools and technologies

**Module 3:** Advanced Applications (${Math.ceil(days/4)} ${unit}s)
• Advanced topics and methodologies
• Real-world problem solving
• Case studies and projects

**Module 4:** Implementation and Assessment (${Math.ceil(days/4)} ${unit}s)
...
The Department imparts **world-class training and research platforms** to students with state-of-the-art computing facilities. Students are exposed to various opportunities such as **in-plant training, internships, and workshops** during their course of study, preparing them for the technical world beyond academics.`;
    ...
    currentY += 4; // Reduced spacing// Registration Procedure (if enabled)
    if (event.registrationProcedure && event.registrationProcedure.enabled) {
      ...
      if (event.registrationProcedure.registrationForm && event.registrationProcedure.registrationForm.enabled) {
        ...
        const fieldHeight = 15; // Reduced from 18
    ...
    if (event.organizingCommittee && event.organizingCommittee.length > 0) {
      ...
      const addCommitteeMemberInColumn = (title, member, x, isInLeftColumn) => {
        if (member) {
          ...
          if (isInLeftColumn) {
            leftColumnY = currentY + 4; // Reduced spacing
          } else {
            rightColumnY = currentY + 4;
          }
          
          doc.setFontSize(8); // Reduced from 9
          doc.setFont('helvetica', 'bold'); // Made name bold
          
          const memberY = isInLeftColumn ? leftColumnY : rightColumnY;
          doc.text(member.name, x + 2, memberY); // Reduced indent
          
          let nextY = memberY + 3; // Reduced spacing
          ...
          if (isInLeftColumn) {
            leftColumnY = nextY + 4; // Reduced gap between members from 6 to 4
          } else {
      ...
      // Find members by role (using actual database roles)
      ...
      currentY = Math.max(leftColumnY, rightColumnY) + 3; // Reduced spacing from 5 to 3
      ...
      if (event.coordinators && event.coordinators.length > 0) {
        ...
        event.coordinators.forEach((coordinator, index) => {
          ...
          doc.setFontSize(8); // Reduced from 9
          doc.setFont('helvetica', 'bold'); // Made coordinator names bold
          doc.text(`${index + 1}. ${coordinator.name}`, x + 2, coordY); // Reduced indent
          
          let nextY = coordY + 3; // Reduced spacing
          ...
          if (isLeftColumn) {
            coordLeftY = nextY + 2; // Reduced spacing
          } else {
            coordRightY = nextY + 2;
          }
        });
      ...
      currentY += 4; // Reduced spacing after organizing committee section
    } else {
