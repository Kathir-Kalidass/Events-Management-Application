...
const EnhancedParticipantDashboard = ({ eventId, eventTitle, userRole = 'coordinator' }) => {
  ...
  const fetchParticipants = async () => {
    try {
      ...
      if (userRole === 'hod') {
        ...
        try {
          const response = await axios.get(`http://10.5.12.1:4000/api/hod/events/${eventId}/participants`, {
        ...
        try {
          const response = await axios.get(`http://10.5.12.1:4000/api/coordinator/events/${eventId}/participants`, {
            headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
          });
  ...
  const handleApproveParticipant = async (participantId) => {
    try {
      await axios.put(
        'http://10.5.12.1:4000/api/coordinator/participants/approve',
  ...
  const handleRejectParticipant = async (participantId) => {
    try {
      await axios.put(
        'http://10.5.12.1:4000/api/coordinator/participants/reject',
  ...
  const handleMarkAttendance = async (participantId, attended) => {
    try {
      await axios.put(
        'http://10.5.12.1:4000/api/coordinator/participants/attendance',
  ...
  const handleBulkApprove = async () => {
    try {
      await axios.put(
        'http://10.5.12.1:4000/api/coordinator/participants/bulk-approve',
  ...
  const handleBulkAttendance = async (attended) => {
    try {
      await axios.put(
        'http://10.5.12.1:4000/api/coordinator/participants/bulk-attendance',
  ...
  const handleSendNotification = async () => {
    try {
      await axios.post(
        'http://10.5.12.1:4000/api/coordinator/participants/notify',
