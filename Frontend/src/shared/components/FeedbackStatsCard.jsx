...
const FeedbackStatsCard = ({ eventId, userRole = 'coordinator' }) => {
  ...
  const fetchFeedbackStats = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:4000/api'}/${baseUrl}/feedback/stats/${eventId}`,
        {
  ...
  const fetchFeedbackDetails = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:4000/api'}/${baseUrl}/feedback/details/${eventId}`,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
