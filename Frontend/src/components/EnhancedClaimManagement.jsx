...
const EnhancedClaimManagement = ({ eventId }) => {
  ...
  const fetchEventData = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5050/api/coordinator/programmes/${eventId}`,
        {
  ...
  const fetchClaims = async () => {
    try {
      ...
      const response = await axios.get(
        eventId 
          ? `http://localhost:5050/api/coordinator/events/${eventId}/claims`
          : `http://localhost:5050/api/coordinator/claims`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        }
      );
  ...
  const fetchStatistics = async () => {
    try {
      const response = await axios.get(
        'http://localhost:5050/api/coordinator/claims/statistics',
        {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        }
      );
  ...
  const handleViewDetails = async (claimId) => {
    try {
      const response = await axios.get(
        `http://localhost:5050/api/coordinator/claims/${claimId}`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        }
      );
  ...
  const submitApproval = async () => {
    try {
      await axios.put(
        `http://localhost:5050/api/coordinator/claims/${selectedClaim._id}/status`,
        {
              ...
              <CardContent>
                <Box display="flex" alignItems="center">
                  ...
                  <Box>
                    ...
                    <Typography variant="h4" color={statistics.amounts?.totalSavings >= 0 ? 'success.main' : 'error.main'}>
                      {formatCurrency(statistics.amounts?.totalSavings)}
                    </Typography>
                  </Box>
                </Box>
              </CardContent>
      ...
      <Card>
        <CardContent>
          <Typography variant="h6" gutterBottom>
            {eventId ? 'Event Claims' : 'All Claims'} ({filteredClaims.length})
          </Typography>
          
          {filteredClaims.length === 0 ? (
            <Alert severity="info">
              {claims.length === 0 ? 'No claims submitted yet' : 'No claims match your search criteria'}
            </Alert>
          ) : (
                      ...
                      <TableRow hover>
                        ...
                        <TableCell>
                          <Typography variant="body2">
                            {claim.eventId?.title || 'N/A'}
                          </Typography>
                          <Typography variant="caption" color="text.secondary">
                            {claim.eventId?.startDate && formatDate(claim.eventId.startDate)}
                          </Typography>
                        </TableCell>
                        <TableCell>
                          <Box display="flex" alignItems="center">
                          ...
                          <Box>
                            ...
                            {claim.summary?.totalSavings !== 0 && (
                              <Box display="flex" alignItems="center" mt={0.5}>
                                ...
                                <Typography 
                                  variant="caption" 
                                  color={claim.summary.totalSavings > 0 ? 'success.main' : 'error.main'}
                                >
                                  {claim.summary.totalSavings > 0 ? 'Saved' : 'Over'}: {formatCurrency(Math.abs(claim.summary.totalSavings))}
                                </Typography>
                              </Box>
                            )}
                          </Box>
                ...
                <Grid item xs={12} md={4}>
                  <Paper sx={{ p: 2, textAlign: 'center' }}>
                    <Typography variant="subtitle2" color="text.secondary">
                      {selectedClaim.summary?.totalSavings >= 0 ? 'Savings' : 'Over Budget'}
                    </Typography>
                    <Typography 
                      variant="h6" 
                      color={selectedClaim.summary?.totalSavings >= 0 ? 'success.main' : 'error.main'}
                    >
...
export default EnhancedClaimManagement;