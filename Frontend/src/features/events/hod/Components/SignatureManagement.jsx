...
const SignatureManagement = () => {
  ...
  const fetchSignature = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:4000/api/hod/signature', {
  ...
  const handleDeleteSignature = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.delete('http://localhost:4000/api/hod/signature', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleActivateSignature = async () => {
    try {
      ...
      const response = await axios.put(
        'http://localhost:4000/api/hod/signature/activate',
        {},
        {
          headers: { Authorization: `Bearer ${token}` }
        }
