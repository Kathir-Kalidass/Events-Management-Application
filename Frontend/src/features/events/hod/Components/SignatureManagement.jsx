...
const SignatureManagement = () => {
  ...
  const fetchSignature = async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL || 'http://10.5.12.1:4000/api'}/hod/signature`, {
  ...
  const handleDeleteSignature = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.delete(`${import.meta.env.VITE_API_BASE_URL || 'http://10.5.12.1:4000/api'}/hod/signature`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleActivateSignature = async () => {
    try {
      ...
      const response = await axios.put(
        `${import.meta.env.VITE_API_BASE_URL || 'http://10.5.12.1:4000/api'}/hod/signature/activate`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` }
        }
