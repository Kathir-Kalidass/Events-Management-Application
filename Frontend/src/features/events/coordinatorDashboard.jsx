...
const CoordinatorDashboard = () => {
  ...
  const getCurrentHod = () => {
    ...
    fetch(`http://10.5.12.1:4000/api/coordinator/getHOD?id=${user._id}`, {
      method: "GET",
      headers: {
  ...
  /*function handleViewFinalBudget(id) {
    ...
    fetch(`http://10.5.12.1:4000/api/coordinator/event/claimPdf/${id}`, {
      method: "GET",
    })
      .then((res) => {
        // Check if response is successful
  ...
  const handleGenerateBrochure = async (eventData) => {
    try {
      ...
      try {
        const response = await axios.get(`http://10.5.12.1:4000/api/hod/events/${eventData._id}`, {
      ...
      try {
        ...
        const token = localStorage.getItem("token");
        await fetch(`http://10.5.12.1:4000/api/coordinator/brochures/${eventData._id}/save`, {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
          },
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          const token = localStorage.getItem("token");
                          const response = await axios.get(
                            `http://10.5.12.1:4000/api/coordinator/programmes/${event._id}/pdf`,
                            {
                              responseType: "blob",
                              headers: {
                                Authorization: `Bearer ${token}`,
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              const response = await axios.get(
                                `http://10.5.12.1:4000/api/coordinator/claims/${event._id}/pdf`,
                                { responseType: "blob" }
                              );
