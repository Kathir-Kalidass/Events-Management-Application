...
const CoordinatorEventDashboard = () => {
  ...
  const fetchEventDetails = async () => {
    try {
      const response = await axios.get(`http://10.5.12.1:4000/api/coordinator/programmes/${eventId}`, {
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this event?")) {
      try {
        await axios.delete(`http://10.5.12.1:4000/api/coordinator/programmes/${eventId}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
  ...
  const downloadCompleteBrochure = async () => {
    try {
      ...
      try {
        const response = await axios.get(`http://10.5.12.1:4000/api/hod/events/${eventId}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
      ...
      try {
        ...
        await fetch(`http://10.5.12.1:4000/api/coordinator/brochures/${event._id}/save`, {
          method: 'POST',
          headers: {
  ...
  const viewProposalPDF = async () => {
    try {
      ...
      const response = await fetch(`http://10.5.12.1:4000/api/coordinator/programmes/${eventId}/pdf`, {
        method: 'GET',
        headers: {
