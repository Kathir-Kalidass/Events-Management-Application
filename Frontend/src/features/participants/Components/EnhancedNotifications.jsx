...
const EnhancedNotifications = ({ notifications = [], onDataChange }) => {
  ...
  const markAsRead = async (notificationId) => {
    try {
      ...
      const response = await fetch(`http://localhost:5050/api/participant/notifications/${notificationId}/read`, {
        method: 'PUT',
        headers: {
  ...
  const markAllAsRead = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5050/api/participant/notifications/${userInfo._id}/mark-all-read`, {
        method: 'PUT',
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const deleteNotification = async (notificationId) => {
    try {
      ...
      const response = await fetch(`http://localhost:5050/api/participant/notifications/${notificationId}`, {
        method: 'DELETE',
        headers: {
          "Authorization": `Bearer ${token}`,
        },
